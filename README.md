...
本系統具備「**修復時間軸與台北天文日出日落自動動態切換日夜模式**」、「**桃園機捷直達特快車經典紫色塗裝 (`#84005C`)**」、「**最高 500 倍超極速播放控制 (`1x`, `5x`, `10x`, `50x`, `100x`, `500x`)**」、「**模擬列車營運時間規範為 06:00 ~ 24:00**」、「**機捷直達特快車於 A13 機場第二航廈迴轉**」、「**模擬模式自訂時間選擇器 (Time Picker)**」、「**官方確切三鶯線 12 車站 (LB01 頂埔 ➔ LB12 鶯桃福德)**」、「**模式切換徹底重置與記憶體清理 (`cleanup()`)**」、「**站對站路徑規劃器 (`RoutePlanner` BFS 最短路徑 + 脈衝動態光點導引動畫)**」等核心功能。
...
│   ├── map.js            # Leaflet 地圖、CartoDB 圖層 (台北天文日出日落 Auto/Manual 觸發)、Zoom 14 站名與 GPS (MapController)
...
- 拖曳左下角時間軸 Slider 或倍速播放時，時間進入 **日出與日落之間 (`05:20` ~ `18:28`)** 地圖自動呈現 **淺色日間模式 (CartoDB Voyager)**，進入 **日落至翌日日出 (`18:28` ~ `05:20`)** 自動呈現 **深色夜間模式 (CartoDB Dark Matter)**。
...
在開發新功能分支（例如 `feature/fix-auto-theme-switching`）時，請遵循以下 Git 操作流程：
...
# 2. 建立並切換至新功能分支
git checkout -b feature/fix-auto-theme-switching

# 3. 進行程式碼開發與測試，確認無誤後 Commit
git status
git add .
git commit -m "fix: resolve auto light/dark theme switching issue on timeline slider adjustments"

# 4. 推送功能分支至 GitHub 遠端
git push -u origin feature/fix-auto-theme-switching

# 5. 切換回 main 分支並進行安全合併 (Merge)
git checkout main
git merge --no-ff feature/fix-auto-theme-switching -m "merge: feature/fix-auto-theme-switching into main"

# 6. 將最新 main 分支推送至 GitHub 遠端
git push origin main

# 7. (選用) 刪除已合併的分支
git branch -d feature/fix-auto-theme-switching
git push origin --delete feature/fix-auto-theme-switching
```
...
1. 推送至 GitHub：
   ```bash
   git add .
   git commit -m "feat: release V17 Fixed Auto Theme Switching SPA"
   git push origin main
   ```
