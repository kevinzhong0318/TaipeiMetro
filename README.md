...
本系統具備「**拉動時間軸 0 毫秒極速無條件自動感應切換淺色/深色地圖**」、「**雙圖層預載 (Dual TileLayers 0ms 瞬間切換日夜模式)**」、「**右上角主題按鈕三階循環 (`🌗自動` ➔ `☀️日間` ➔ `🌙夜間`)**」、「**桃園機捷直達特快車經典紫色塗裝 (`#84005C`)**」、「**最高 500 倍超極速播放控制 (`1x`, `5x`, `10x`, `50x`, `100x`, `500x`)**」、「**模擬列車營運時間規範為 06:00 ~ 24:00**」、「**機捷直達特快車於 A13 機場第二航廈迴轉**」、「**模擬模式自訂時間選擇器 (Time Picker)**」、「**官方確切三鶯線 12 車站 (LB01 頂埔 ➔ LB12 鶯桃福德)**」、「**模式切換徹底重置與記憶體清理 (`cleanup()`)**」、「**站對站路徑規劃器 (`RoutePlanner` BFS 最短路徑 + 脈衝動態光點導引動畫)**」等核心功能。
...
│   ├── timelineController.js # 時間軸 Slider (拉動時間軸無條件 0ms 即刻切換主題) 與 500x 倍速控制器 (TimelineController)
...
### 1. ⚡ 時間軸無條件 0 毫秒極速自動感應 (Unconditional Instant Theme Switch)
- 修正原先 `themeMode` 被手動狀態鎖定問題。當拖拉左下角時間軸、輸入時間或進行 1x~500x 播放時，無條件直連呼叫 `updateAutoTheme(currentDate, true)`。
- 時間在日出後 (如 `10:37`) 地圖 **0 毫秒即刻瞬間切換為淺色日間模式 (CartoDB Voyager)**，在日落後 (如 `22:00`) 地圖 **0 毫秒即刻瞬間切換為深色夜間模式 (CartoDB Dark Matter)**。

### 2. ⚡ 雙圖層預載技術 (Instant Dual TileLayers 0ms Swap)
- 地圖初始化時同時預加載 `darkTileLayer` 與 `lightTileLayer`，調控 Opacity (1/0) 達成瞬間無縫切換。

---

## 🌿 Standard Git Branching Workflow 指引

在開發新功能分支（例如 `feature/fix-timeline-unconditional-theme-switch`）時，請遵循以下 Git 操作流程：
...
# 2. 建立並切換至新功能分支
git checkout -b feature/fix-timeline-unconditional-theme-switch

# 3. 進行程式碼開發與測試，確認無誤後 Commit
git status
git add .
git commit -m "fix: resolve timeline slider theme switching by adding unconditional force trigger"

# 4. 推送功能分支至 GitHub 遠端
git push -u origin feature/fix-timeline-unconditional-theme-switch

# 5. 切換回 main 分支並進行安全合併 (Merge)
git checkout main
git merge --no-ff feature/fix-timeline-unconditional-theme-switch -m "merge: feature/fix-timeline-unconditional-theme-switch into main"

# 6. 將最新 main 分支推送至 GitHub 遠端
git push origin main

# 7. (選用) 刪除已合併的分支
git branch -d feature/fix-timeline-unconditional-theme-switch
git push origin --delete feature/fix-timeline-unconditional-theme-switch
```
...
1. 推送至 GitHub：
   ```bash
   git add .
   git commit -m "feat: release V19 Unconditional Theme Switching SPA"
   git push origin main
   ```
